...
- **通知**: 新しいターミナルを開いたとき(環境設定ステップが ~/.bashrc に入れる更新チェック)と、
  Webアプリの画面上部バナーの2箇所で「新しいリリースがあります」と知らせます。
  どのバージョンを入れたか覚えておく必要はありません(gitのHEADがバージョンそのものです)
- **更新の基準はリリースタグ**(`v*`)です。ブランチに積まれただけのコミットでは通知されず、
  リリースが打たれたときだけ利用者に知らせます(タグが無いリポジトリはブランチ比較に
  フォールバック)
- **実行**: バナーの「アップデート」ボタン(リリース地点までfast-forward)→ run.shを再起動。
  ロボットソースの更新はROS導入ステップの「ロボットソフトをアップデート」ボタン
  (取得+再ビルド)。処理内容が変わったステップには「更新あり」バッジが付くので、
  そのステップを「もう一度実行」すれば反映されます(全ステップ再実行安全)

### 管理者向け: リリースの出し方

利用者に更新を知らせるには、annotated tag を打ってpushします:

```bash
git tag -a v1.2.0 -m "センサ確認が固まる問題を修正しました"
git push origin v1.2.0
```

**`-m` のメッセージがそのまま利用者のバナー・ターミナル通知に表示されます**。
利用者向けの言葉で(内部用語を避けて)書いてください。複数行も可です。
